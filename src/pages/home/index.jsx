...
const Home = () => {
                            ...
                            policyNumber:
                              response?.data?.data?.customerEntitlement[0]?.policyNumber
                                ...
                                ? response?.data?.data?.customerEntitlement[0]
                                    ?.policyNumber
                                : null,
                            policyStartDate: res?.data?.data[0]?.policyStartDate
                              ? moment(
    ...
    tempCaseMutate(
      {
        ...
        ...(quickSearchData?.data?.callInitiationId && {
          callInitiationId: quickSearchData?.data?.callInitiationId,
        }),
      },
      {
        onSuccess: (res) => {
    ...
    tempCaseMutate(
      {
        ...
        vin:
          record && record?._source?.vin?.toString()?.toLowerCase() !== "null"
            ...
            : null,
        contactName:
          quickSearchData?.data?.data?.customer?.length > 0
            ? quickSearchData?.data?.data?.customer[0]?._source?.policies
                ?.name || null
            : null,
        mobileNumber:
          quickSearchData?.data?.data?.customer?.length > 0
            ? quickSearchData?.data?.data?.customer[0]?._source
                ?.mobileNumber !== "NA"
              ? quickSearchData?.data?.data?.customer[0]?._source?.mobileNumber.toString() ||
                null
              : null
            : null,
        channelId: null,
        ...
        ...(((quickSearchData?.success &&
          ...
          policyNumber:
            searchPayload?.policyNumber ||
            quickSearchData?.data?.data?.customerEntitlement[0]?.policyNumber,
        }),
        ...(quickSearchData?.data?.callInitiationId && {
          callInitiationId: quickSearchData?.data?.callInitiationId,
        }),
      ...
      : [
          {
            ...
            body: (record, field) => (
              <>
                {" "}
                {!quick && permissions?.includes("create-case") && (
                  <Button
                    type="button"
                    className="btn-link"
  ...
  const CaseHistoryColumns = [
    ...
    },
    {
      ...
      field: "_source",
      body: (record, field) => {
        return <>{record?._source?.irateCustomer || "--"}</>;
      },
    },
    {
      title: "Vehicle Number",
  ...
  useEffect(() => {
    if (
      quickSearchData?.data?.success &&
      quickSearchData?.data?.data?.customerEntitlement?.length > 0
    ) {
      entitlementMutate(
        {
          clientId:
          ...
          vin: quickSearchData?.data?.data?.customerEntitlement[
            activeEntitlementIndex
          ]?.vin,
          vehicleRegistrationNumber:
            quickSearchData?.data?.data?.customerEntitlement[
  ...
  return (
    <div className="page-wrap">
      <div className="page-body d-flex flex-column">
        ...
        {!showSearchResults &&
          permissions?.includes("agent-ongoing-cases-web") && (
              ...
              <div className="col-md-8 col-sm-12">
                <div className="custom-card h-100 padding">
                  <div className="custom-card-header d-flex align-items-center gap">
                    <div>
                      My Ongoing Cases{" "}
                      <Badge value={data?.data?.data?.length}></Badge>
                    </div>
                      ...
                      {searchResults?.data?.vehicles?.length > 0 ? (
                        <>
                          {searchResults?.data?.vehicles?.length > 1 ? (
                            <>
                              {!quick &&
                                permissions?.includes("create-case") && (
                                  <SplitButton
