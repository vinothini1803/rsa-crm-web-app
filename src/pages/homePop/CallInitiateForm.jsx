...
import { getFormDataCallInitiation, getFormDataDisposition, createCallIntiate } from "../../../services/caseService";
import { useNavigate } from "react-router";
import { checkSupport, firebaseToken, onMessageListener } from "../../../services/firebaseService";
import { SearchCall } from "../../../store/slices/searchSlice";

const CallInitiateForm = ({ onSearch, clientOptions, selectedAccount, onSelectedAccount, submitLoading, initialSearchCaseData, showSearchResults, setShowSearchResults, hasOpened }) => {
  const defaultValues = {
    selectAccount: '',
    subjectId: [],
    dispositionId: "",
  ...
  const { handleSubmit, control, setValue, resetField, formState: { errors }, reset, getValues } = useForm({ defaultValues });
...
const hasOpenedRef = useRef(false);
  ...
  const {isLoading: getFormDataLoading} = useQuery(["getFormDataCallInitiation"], () => getFormDataCallInitiation({id:''}), {
    onSuccess: (response) => {
      ...
      if(response?.data?.success) {
        ...
        console.log("Storing formData in sessionStorage:", response.data.data);
      } else {
  ...
  const onSubmit = (values) => {
    ...
    console.log("Search Component values", values?.subjectId?.includes(392)  && values?.dispositionId?.id == 3);
    if(values?.subjectId?.includes(392)  && values?.dispositionId?.id == 3) {
...
useEffect(() => {
  // if (!searchData || hasSearchedRef.current) return;

  // const relevantFields = ({ client, contactNumber }) => ({ client, contactNumber });

  // const hasChanged = !prevSearchDataRef.current || 
  //   JSON.stringify(relevantFields(prevSearchDataRef.current)) !== JSON.stringify(relevantFields(searchData));

  // console.log("Has searchData changed?", hasChanged);
  // console.log("Has popup opened before?", hasOpened);

  // if (!hasOpened && !hasChanged) return; // Prevent onSubmit on reopens without new data
...
const last10Digits = mobileNumber ? mobileNumber.slice(-10) : "";
  ...
  useEffect(() => {
    if(initialSearchCaseData) {
      ...
      const otherFields = searchField.filter(name => !incomingSearchFields?.includes(name));
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
              ...
              render={({ field, fieldState }) => (
                <Dropdown
                  ...
                  onChange={(e) => {
                    ...
                    setFieldEdited(fieldEdited?.length == searchField?.length ? [] : fieldEdited);
                  }}
                />
              )}
            />
        ...
        <div className="custom-card-body mt-2_3">
          <div className="row row-gap-3_4">
            ...
            <div className="col-md-12">
              <Button type="submit" className="btn btn-primary" loading={submitLoading} disabled={selectedAccount ? false : true}>
