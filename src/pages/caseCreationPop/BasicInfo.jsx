...
const BasicInfo = ({formOptions, formErrors, entitlementData, client, setFullEntitlementVisible, existCustomer,methods,vehicleRegistrationNumber,vin}) => {
...
console.log("basicVinNumber",vehicleNumber =="")
  ...
  const {data: vinData, mutate: vinMutate, isLoading: vinLoading} = useMutation(getVIN)
const handleVIN=()=>{
  vinMutate({
    vehicleRegistrationNumber: vehicleNumber,
      ...
      onSuccess: (res) => {
        if(res?.data?.success){
         methods.setValue("basicVinNumber",res?.data?.vehicleDetails?.viNumber)
           let FuelValue= res?.data?.fuelTypes?.find((item)=>item?.name?.toLowerCase() ===res?.data?.vehicleDetails?.fuelType?.toLowerCase())?.id || null
    ...
    <div className="row row-gap-3_4">
      <div className="col-md-6">
        <div className="row row-gap-3_4">
          ...
          <div className="col-md-6">
            <div className="form-group">
              <label className="form-label required">Vehicle Manufacturer</label>
              <Controller
              ...
              <Controller
                ...
                rules={{ 
                  ...
                  validate: {
                    matchPattern: (v) =>{
                      if(v?.length > 0) {
                        return /^\d+(\.\d{1,2})?$/.test(v) || 'KM must be positive number with up to two decimal places';
                      } else {
                        return Promise.resolve();
                      }
                    }
          ...
          {selectedPolicyType?.id !== 434 && (
            <>
              ...
              <div className="col-md-6">
                <div className="form-group">
                  <label className="form-label required">W/EW/RSA Pol Start Date</label>
                  <Controller
              ...
              <div className="col-md-6">
                <div className="form-group">
                  <label className="form-label required">W/EW/RSA Pol End Date</label>
                  <Controller
                    name="policyEndDate"
                    control={control}
        ...
        <div className="row row-gap-3_4 case-position-sticky">
          <div className="col-md-12">
            ...
            <ServiceDetailCard 
              ...
              entitlementData={{
                policyData: selectedPolicyType?.id !== 434 ? {
                  policyNumber: selectedPolicyNumber,
                  policyStartDate: moment(selectedPolicyStart).format("DD/MM/YYYY"),
                  policyEndDate: moment(selectedPolicyEnd).format("DD/MM/YYYY"),
                } : null,
          ...
          {(entitlementData?.notes && entitlementData?.notes?.message?.length > 0) && 
            <div className="col-md-12">
              <Note
                ...
                title={true}
              >
                <div className={`note-content ${entitlementData?.notes?.status ? "note-content-green" : ''}`}>
