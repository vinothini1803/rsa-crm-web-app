...
import { AspRejectCcIcon, ImportFileBaseUrl, } from "../../../utills/imgConstants";
import { RadioButton } from "primereact/radiobutton";
import { toast } from "react-toastify";
import { useMutation, useQuery } from "react-query";
import {
    saveAspRejectedCcDetailReason,
    aspRejectedCcDetailReasonFormData,
    aspRejectedCcDetailReason,
    deleteAspRejectedCcDetailReason,
    updateAspRejectedCcDetailReasonStatus,
    aspRejectedCcDetailReasonImportMaster,
    aspRejectedCcDetailReasonExportMaster
...
const AspRejectedCcDetailReason = () => {
    ...
    const { data: formData, refetch: formDataRefetch } = useQuery(
        ...
        () =>
            aspRejectedCcDetailReasonFormData({
                aspRejCcReasonId: selectedRows[0].id,
            }),
        {
            enabled: false,
            onSuccess: (res) => {
                if (res?.data?.success) {
                    if (selectedRows?.length == 1) {
                        ...
                        Object.keys(defaultValues)?.forEach((field) =>
                            setValue(`${field}`, res?.data?.data?.aspRejCcReason[field])
                        );
                    }
                } else {
                    if (res?.data?.error) {
    ...
    const { mutate, isLoading } = useMutation(saveAspRejectedCcDetailReason);
    const { mutate: statusMutate, isLoading: statusIsLoading } =
        useMutation(updateAspRejectedCcDetailReasonStatus);
    const { mutate: deletemutate, isLoading: deleteIsLoading } =
        useMutation(deleteAspRejectedCcDetailReason);
    const { mutate: importMutate, isLoading: importIsLoading } =
        useMutation(aspRejectedCcDetailReasonImportMaster);
    const { mutate: exportMutate, isLoading: exportIsLoading } =
        useMutation(aspRejectedCcDetailReasonExportMaster);

    const handleStatusUpdate = (records, status, setVisible) => {
        console.log(records, status);
        statusMutate(
            {
                aspRejCcReasonIds: records?.map((row) => row.id),
                status: status,
            },
            {
                onSuccess: (res) => {
    ...
    const columns = [
        { title: "Asp Rejected Cc Detail Reason", field: "name" },
    ...
    const handleFormSubmit = (values) => {
        mutate(
            {
                aspRejCcReasonId: selectedRows?.length == 1 ? selectedRows[0]?.id : null,
                ...values,
    ...
    const handleDelete = (records, setVisible) => {
        ...
        deletemutate(
            {
                aspRejCcReasonIds: records?.reduce((acc, el) => acc.concat(el.id), []),
            },
            {
                onSuccess: (res) => {
                    if (res?.data?.success) {
        ...
        exportMutate(values, {
            onSuccess: (res) => {
                if (res?.data?.success) {
                    ...
                    reset();
                    if (res?.data?.path) {
                        download(`${ImportFileBaseUrl}${res?.data?.path}`, "ASP rejected CC Detail Reason");
                    }
                } else {
                    if (res?.data?.error) {
                        toast.error(res?.data?.error);
    ...
    return (
        <div className="page-wrap">
            <div className="page-body">
                <TableWrapper
                    title={"Asp Rejected Cc Detail Reason"}
                    rowSelection
                    columns={columns}
            ...
            <ImportDialog
                ...
                sampleExcel={`${ImportFileBaseUrl}/importTemplates/aspRejectedCcDetailReason_import_template.xlsx`}
            ...
            <Dialog
                header={
                    <div className="dialog-header">
                        <div className="dialog-header-title">Asp Rejected Cc Detail Reason</div>
                    </div>
                }
                visible={visible}
                position={"bottom"}
                        ...
                        <div className="col-md-12">
                            <div className="form-group">
                                <div lassName="form-group">
                                    <label className="form-label required">Asp Rejected Cc Detail Reason</label>
                                    <Controller
                                        name="name"
                                        rules={{
                                            required: "Asp Rejected Cc Detail Reason is required.",
                                        }}
                                        control={control}
                                        render={({ field, fieldState }) => (
                                            <>
                                                <InputText {...field} placeholder="Enter Asp Rejected Cc Detail Reason" />
