...
import {
  towFailureReasons,
  deleteTowFailureReason,
  towFailureReasonFormData,
  saveTowFailureReason,
  updateTowFailureReasonStatus,
  towFailureReasonImport,
  towFailureReasonExport,
...
const TowFailureReasons = () => {
  ...
  const [towFailureReasonId, setTowFailureReasonId] = useState(null);
  ...
  const { data, isFetching, refetch } = useQuery(
    ["towFailureReasonList", pagination, searchValue, filters],
    () =>
      towFailureReasons({
        apiType: "list",
        ...pagination,
  ...
  const { mutate, isLoading } = useMutation(saveTowFailureReason);
  ...
  const { data: towFailureReasonData, refetch: formDataRefetch } = useQuery(
    ["towFailureReasonFormData"],
    () =>
      towFailureReasonFormData({
        towFailureReasonId: selectedRows[0].id,
      }),
    {
      enabled: false,
      onSuccess: (res) => {
        if (res?.data?.success) {
          const failureData = res?.data?.data?.towFailureReason;
  ...
  const columns = [
    { title: "Tow Failure Reason", field: "name" },
  ...
  const handleAdd = () => {
    setVisible(true);
    setTowFailureReasonId(null);
  };
  const handleEdit = (record) => {
    setVisible(true);
    formDataRefetch();
    setTowFailureReasonId(record?.id);
  };
  ...
  const handleFormSubmit = (values) => {
    mutate(
      {
        ...(towFailureReasonId && {
          towFailureReasonId: towFailureReasonId,
        }),
        ...values,
  ...
  const handleDelete = (records, setVisible) => {
    deletemutate(
      {
        towFailureReasonIds: records?.reduce(
  ...
  const handleStatusUpdate = (records, status, setVisible) => {
    statusMutate(
      {
        towFailureReasonIds: records?.map((row) => row.id),
        status: status,
      },
      {
        onSuccess: (res) => {
  ...
  return (
    <div className="page-wrap">
      ...
      <ImportDialog
        ...
        sampleExcel={`${ImportFileBaseUrl}/importTemplates/towFailureReason_import_template.xlsx`}
        masterType={"towFailureReason"}
        ...
        header={
          <div className="dialog-header">
            <div className="dialog-header-title">
              {towFailureReasonId ? "Edit" : "Add"} Tow Failure Reason
            </div>
          </div>
        }
        visible={visible}
...
export default TowFailureReasons;
