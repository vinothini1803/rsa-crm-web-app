...
const Taluk = () => {
  ...
  const { data, isFetching, refetch } = useQuery(
    ["talukList", pagination, searchValue, filters],
    () =>
      talukList({
        apiType: "list",
        ...pagination,
  ...
  const { mutate, isLoading } = useMutation(saveTaluk);
  const { data: talukFormData } = useQuery(
    ["talukFormData", selectedRows],
    () =>
      getTalukData({
        talukId: selectedRows?.length == 1 ? selectedRows[0]?.id : "",
      }),
    {
        ...
        if (res?.data?.success) {
          if (selectedRows?.length == 1) {
            ...
            Object.keys(defaultValues)?.forEach((field) =>
              setValue(`${field}`, res?.data?.data?.taluk[field])
            );
          }
        } else {
          if (res?.data?.error) {
  ...
  const handleFormSubmit = (values) => {
    mutate(
      {
        talukId: selectedRows?.length == 1 ? selectedRows[0]?.id : null,
        ...values,
  ...
  const handleStatusUpdate = (records, status, setVisible) => {
    ...
    statusMutate(
      {
        talukIds: records?.map((row) => row.id),
        status: status,
      },
      {
        onSuccess: (res) => {
    ...
    exportMutate(values, {
      onSuccess: (res) => {
        if (res?.data?.success) {
          ...
          reset();
          if (res?.data?.path) {
            download(`${ImportFileBaseUrl}${res?.data?.path}`, "Taluk");
          }
        } else {
          if (res?.data?.error) {
            toast.error(res?.data?.error);
  ...
  return (
    <>
      ...
      <ImportDialog
        ...
        sampleExcel={`${ImportFileBaseUrl}/importTemplates/taluk_import_template.xlsx`}
        masterType={"taluk"}
      ...
      <Dialog
        ...
        header={
          <div className="dialog-header">
            <div className="dialog-header-title">{talukId ? "Edit":"Add"} Taluk</div>
          </div>
        }
        visible={visible}
        position={"bottom"}
          ...
          <div className="row row-gap-3_4">
            ...
            <div className="col-md-6">
              <div className="form-group">
                <label className="form-label required">Taluk Name</label>
                <Controller
                  name="name"
                  rules={{
                    required: "Taluk Name is required.",
                  }}
                  control={control}
                  render={({ field, fieldState }) => (
                    <>
                      <InputText {...field} placeholder="Enter Taluk Name" />
