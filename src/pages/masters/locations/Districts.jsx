...
import {
  districtImportMaster,
  districtExportMaster,
  getDistrictData,
  deleteDistrict,
  districtStatusUpdate,
  districtList,
  saveDistrict
...
const District = () => {
  ...
  const { data, isFetching, refetch } = useQuery(
    ["districtList", pagination, searchValue, filters],
    () =>
      districtList({
        apiType: "list",
        ...pagination,
  ...
  const { mutate, isLoading } = useMutation(saveDistrict);
  const { data: districtFormData } = useQuery(
    ["districtFormData", selectedRows],
    () =>
      getDistrictData({
        districtId: selectedRows?.length == 1 ? selectedRows[0]?.id : "",
      }),
    {
        ...
        if (res?.data?.success) {
          if (selectedRows?.length == 1) {
            ...
            Object.keys(defaultValues)?.forEach((field) =>
              setValue(`${field}`, res?.data?.data?.district[field])
            );
          }
        } else {
          if (res?.data?.error) {
  ...
  const handleFormSubmit = (values) => {
    mutate(
      {
        districtId: selectedRows?.length == 1 ? selectedRows[0]?.id : null,
        ...values,
  ...
  const handleDelete = (records, setVisible) => {
    ...
    deletemutate(
      {
        districtIds: records?.reduce((acc, el) => acc.concat(el.id), []),
      },
      {
        onSuccess: (res) => {
          if (res?.data?.success) {
  ...
  const handleStatusUpdate = (records, status, setVisible) => {
    ...
    statusMutate(
      {
        districtIds: records?.map((row) => row.id),
        status: status,
      },
      {
        onSuccess: (res) => {
    ...
    exportMutate(values, {
      onSuccess: (res) => {
        if (res?.data?.success) {
          ...
          reset();
          if (res?.data?.path) {
            download(`${ImportFileBaseUrl}${res?.data?.path}`, "Districts");
          }
        } else {
          if (res?.data?.error) {
            toast.error(res?.data?.error);
  ...
  return (
    <>
      ...
      <ImportDialog
        ...
        sampleExcel={`${ImportFileBaseUrl}/importTemplates/district_import_template.xlsx`}
        masterType={"district"}
      ...
      <Dialog
        ...
        header={
          <div className="dialog-header">
            <div className="dialog-header-title">{districtId ? "Edit":"Add"} District</div>
          </div>
        }
        visible={visible}
        position={"bottom"}
              ...
              <div className="form-group">
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <>
                      <InputText {...field} placeholder="Enter District Name" />
