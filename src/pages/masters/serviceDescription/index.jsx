...
const ServiceDescription = () => {
  ...
  const [serviceDescriptionId, setServiceDescriptionId] =
    useState(null);
  ...
  const { data, isFetching, refetch } = useQuery(
    ["getServiceDescription", pagination, searchValue, filters],
    () =>
      getServiceDescription({
        apiType: "list",
        ...pagination,
  ...
  const { mutate, isLoading } = useMutation(saveServiceDescription);
  ...
  const { data: serviceDescriptionForm, refetch: formDataRefetch } = useQuery(
    ["serviceDescriptionData"],
    () =>
      serviceDescriptionData({
        serviceDescriptionId: selectedRows[0].id,
      }),
    {
      enabled: false,
      onSuccess: (res) => {
        if (res?.data?.success) {
          Object.keys(defaultValues)?.forEach((field) =>
            setValue(
              `${field}`,
              res?.data?.data?.serviceDescription[field]
            )
          );
        } else {
          if (res?.data?.error) {
  ...
  const { mutate: importMutate, isLoading: importIsLoading } =
    useMutation(serviceDescriptionImport);

  const { mutate: exportMutate, isLoading: exportIsLoading } =
    useMutation(serviceDescriptionExport);
  ...
  const handleAdd = () => {
    setVisible(true);
    setServiceDescriptionId(null);
  };
  const handleEdit = (record) => {
    setVisible(true);
    formDataRefetch();
    setServiceDescriptionId(record?.id);
  };
  ...
  const handleDelete = (records, setVisible) => {
    deletemutate(
      {
        serviceDescriptionIds: records?.reduce(
  ...
  const handleStatusUpdate = (records, status, setVisible) => {
    statusMutate(
      {
        serviceDescriptionIds: records?.map((row) => row.id),
        status: status,
      },
      {
        onSuccess: (res) => {
    ...
    exportMutate(values, {
      onSuccess: (res) => {
        if (res?.data?.success) {
          ...
          reset();
          if (res?.data?.path) {
            download(`${ImportFileBaseUrl}${res?.data?.path}`, "Service Description");
          }
        } else {
          if (res?.data?.error) {
            toast.error(res?.data?.error);
  ...
  return (
    <div className="page-wrap">
      ...
      <ImportDialog
        ...
        sampleExcel={`${ImportFileBaseUrl}/importTemplates/serviceDescription_import_template.xlsx`}
        masterType={"serviceDescription"}
      ...
      <Dialog
        header={
          <div className="dialog-header">
            <div className="dialog-header-title">{serviceDescriptionId ? "Edit" :"Add"} Service Description</div>
          </div>
        }
        visible={visible}
        position={"bottom"}
              ...
              <div className="form-group">
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <>
                      <InputText placeholder="Enter Service Description" {...field} />
