...
const VehicleModel = () => {
  ...
  const [vehicleModelId, setVechicleModelId] = useState("");
  ...
  const { data, isFetching, refetch } = useQuery(
    ["vehicleModelList", searchValue, pagination, filters],
    () =>
      vehicleModal({
        apiType: "list",
        ...pagination,
        ...
        if (res?.data?.success) {
          if (vehicleModelId) {
            Object.keys(defaultValues)?.forEach((field) =>
              setValue(`${field}`, res?.data?.data?.vehicleModel[field])
            );
          }
        }
      },
  ...
  const handleEdit = () => {
    setVisible(true);
    setVechicleModelId(selectedRows[0]?.id);
  };

  const columns = [
    { title: "Vehicle Model Name", field: "name" },
    { title: "Vehicle Manufacturer Name", field: "vehicleMakeName" },
    {
      title: "Vehicle Type",
      field: "vehicleTypeName",
      body: (record, field) => record.vehicleTypeName ?? "--",
    },
  ...
  const handleStatusUpdate = (records, status, setVisible) => {
    ...
    statusMutate(
      {
        vehicleModelIds: records?.map((row) => row.id),
        status: status,
      },
      {
        onSuccess: (res) => {
  ...
  const handleDelete = (records, setVisible) => {
    ...
    deletemutate(
      {
        vehicleModelIds: records?.reduce((acc, el) => acc.concat(el.id), []),
      },
      {
        onSuccess: (res) => {
          if (res?.data?.success) {
    ...
    exportMutate(values, {
      onSuccess: (res) => {
        if (res?.data?.success) {
          ...
          reset();
          if (res?.data?.path) {
            download(`${ImportFileBaseUrl}${res?.data?.path}`, "Vehicle Model");
          }
        } else {
          if (res?.data?.error) {
            toast.error(res?.data?.error);
  ...
  return (
    <div className="page-wrap">
      ...
      <ImportDialog
        ...
        sampleExcel={`${ImportFileBaseUrl}/importTemplates/vehicle_model_import_template.xlsx`}
        ...
        header={
          <div className="dialog-header">
            <div className="dialog-header-title">
              {vehicleModelId !== "" ? "Edit" : "Add"} Vehicle Model
            </div>
          </div>
        }
        visible={visible}
            ...
            <div className="col-md-6">
              <div className="form-group">
                ...
                <Controller
                  ...
                  control={control}
                  rules={{
                    required: "Vehicle Model Name is required.",
                  }}
                  render={({ field, fieldState }) => (
                    <>
                      <InputText {...field} placeholder="Enter Model Name" />
            ...
            <div className="col-md-6">
              <div className="form-group">
                ...
                <Controller
                  name="vehicleMakeId"
                  control={control}
                  rules={{ required: "Vehicle Manufacturer Name is required." }}
                  render={({ field, fieldState }) => (
                    <>
                      <Dropdown
                        value={field.value}
                        placeholder="Select"
                        options={formData?.data?.data?.extras?.vehicleMakes}
                        optionLabel="name"
                        optionValue="id"
                  ...
                  render={({ field, fieldState }) => (
                    <>
                      <Dropdown
                        value={field.value}
                        placeholder="Select"
                        options={formData?.data?.data?.extras?.vehicleTypes}
                        optionLabel="name"
                        optionValue="id"
                        onChange={(e) => field.onChange(e.value)}
                        filter
