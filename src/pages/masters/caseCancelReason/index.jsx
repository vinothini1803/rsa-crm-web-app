...
import {
  getCaseCancelReason,
  saveCaseCancelReason,
  deleteCaseCancelReason,
  caseCancelReasonData,
  updateCaseCancelReason,
  caseCancelReasonExport,
  caseCancelReasonImport
...
const CaseCancelReason = () => {
  ...
  const [caseCancelId, setCaseCancelId] =
    useState(null);
  ...
  const { data, isFetching, refetch } = useQuery(
    ["getCaseCancelReason", pagination, searchValue, filters],
    () =>
      getCaseCancelReason({
        apiType: "list",
        ...pagination,
  ...
  const { mutate, isLoading } = useMutation(saveCaseCancelReason);
  ...
  const { data: caseCancelReasonForm, refetch: formDataRefetch } = useQuery(
    ["caseCancelReasonData"],
    () =>
      caseCancelReasonData({
        caseCancelReasonId: selectedRows[0].id,
      }),
    {
      enabled: false,
      onSuccess: (res) => {
        if (res?.data?.success) {
          ...
          Object.keys(defaultValues)?.forEach((field) =>
            setValue(
              `${field}`,
              res?.data?.data?.caseCancelReason[field]
            )
          );
        } else {
          if (res?.data?.error) {
  ...
  const { mutate: importMutate, isLoading: importIsLoading } =
    useMutation(caseCancelReasonImport);

  const { mutate: exportMutate, isLoading: exportIsLoading } =
    useMutation(caseCancelReasonExport);
  ...
  const handleAdd = () => {
    setVisible(true);
    setCaseCancelId(null);
  };
  const handleEdit = (record) => {
    setVisible(true);
    formDataRefetch();
    setCaseCancelId(record?.id);
  };
  ...
  const handleFormSubmit = (values) => {
    mutate(
      {
        ...(caseCancelId && {
          caseCancelReasonId: caseCancelId,
        }),
        ...values,
  ...
  const handleDelete = (records, setVisible) => {
    ...
    deletemutate(
      {
        caseCancelReasonIds: records?.reduce(
  ...
  const handleStatusUpdate = (records, status, setVisible) => {
    ...
    statusMutate(
      {
        caseCancelReasonIds: records?.map((row) => row.id),
        status: status,
      },
      {
        onSuccess: (res) => {
    ...
    exportMutate(values, {
      onSuccess: (res) => {
        if (res?.data?.success) {
          ...
          reset();
          if (res?.data?.path) {
            download(`${ImportFileBaseUrl}${res?.data?.path}`, "Case Cancel Reason");
          }
        } else {
          if (res?.data?.error) {
            toast.error(res?.data?.error);
  ...
  return (
    <div className="page-wrap">
      ...
      <ImportDialog
        ...
        sampleExcel={`${ImportFileBaseUrl}/importTemplates/caseCancelReason_import_template.xlsx`}
        masterType={"caseCancelReason"}
      ...
      <Dialog
        header={
          <div className="dialog-header">
            <div className="dialog-header-title">{caseCancelId ? "Edit":"Add"} Case Cancel Reason</div>
          </div>
        }
        visible={visible}
        position={"bottom"}
          ...
          <div className="row row-gap-3_4">
            <div className="col-md-12">
              <div className="form-group">
                <label className="form-label required">Case Cancel Reason</label>
                <Controller
                  name="name"
                  control={control}
                  rules={{
                    required: "Case Cancel Reason required.",
                  }}
                  render={({ field, fieldState }) => (
                    <>
                      <InputText placeholder="Enter Case Cancel Reason" {...field} />
