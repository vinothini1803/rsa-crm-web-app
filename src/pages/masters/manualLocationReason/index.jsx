...
import {
  getManualLocationReason,
  saveManualLocationReason,
  deleteManualLocationReason,
  manualLocationReasonData,
  updateManualLocationReason,
  manualLocationReasonExport,
  manualLocationReasonImport
...
const ManualLocationReason = () => {
  ...
  const [manualLocationId, setManualLocationId] =
    useState(null);
  ...
  const { data, isFetching, refetch } = useQuery(
    ["getManualLocationReason", pagination, searchValue, filters],
    () =>
      getManualLocationReason({
        apiType: "list",
        ...pagination,
  ...
  const { mutate, isLoading } = useMutation(saveManualLocationReason);
  ...
  const { data: manualLocationReasonForm, refetch: formDataRefetch } = useQuery(
    ["manualLocationReasonData"],
    () =>
      manualLocationReasonData({
        manualLocationReasonId: selectedRows[0].id,
      }),
    {
      enabled: false,
      onSuccess: (res) => {
        if (res?.data?.success) {
          ...
          Object.keys(defaultValues)?.forEach((field) =>
            setValue(
              `${field}`,
              res?.data?.data?.manualLocationReason[field]
            )
          );
        } else {
          if (res?.data?.error) {
  ...
  const { mutate: importMutate, isLoading: importIsLoading } =
    useMutation(manualLocationReasonImport);

  const { mutate: exportMutate, isLoading: exportIsLoading } =
    useMutation(manualLocationReasonExport);
  ...
  const handleFormSubmit = (values) => {
    mutate(
      {
        ...(manualLocationId && {
          manualLocationReasonId: manualLocationId,
        }),
        ...values,
  ...
  const handleDelete = (records, setVisible) => {
    ...
    deletemutate(
      {
        manualLocationReasonIds: records?.reduce(
  ...
  const handleStatusUpdate = (records, status, setVisible) => {
    ...
    statusMutate(
      {
        manualLocationReasonIds: records?.map((row) => row.id),
        status: status,
      },
      {
        onSuccess: (res) => {
    ...
    exportMutate(values, {
      onSuccess: (res) => {
        if (res?.data?.success) {
          ...
          reset();
          if (res?.data?.path) {
            download(`${ImportFileBaseUrl}${res?.data?.path}`, "Manual Location Reason");
          }
        } else {
          if (res?.data?.error) {
            toast.error(res?.data?.error);
  ...
  return (
    <div className="page-wrap">
      ...
      <ImportDialog
        ...
        sampleExcel={`${ImportFileBaseUrl}/importTemplates/manual_location_reason_import_template.xlsx`}
        masterType={"manualLocationReason"}
      ...
      <Dialog
        header={
          <div className="dialog-header">
            <div className="dialog-header-title">{manualLocationId ? "Edit" : "Add"} Manual Location Reason</div>
          </div>
        }
        visible={visible}
        position={"bottom"}
          ...
          <div className="row row-gap-3_4">
            <div className="col-md-12">
              <div className="form-group">
                <label className="form-label required">Manual Location Reason</label>
                <Controller
                  name="name"
                  control={control}
                  rules={{
                    required: "Manual Location Reason required.",
                  }}
                  render={({ field, fieldState }) => (
                    <>
                      <InputText placeholder="Enter Manual Location Reason" {...field} />
